    ...
    >
      ...
      <motion.div variants={revealItem} className="mt-10">
        <div className="mb-4 flex items-center gap-3">
          ...
          <p className="font-mono-tag text-[11px] tracking-[0.2em] text-dark/50">
            SEND A MESSAGE — OUR TEAM LEADS GET BACK TO YOU ASAP
          </p>
          <span className="h-px flex-1 bg-dark/10" />
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {whatsappLeads.map((w) => (
            <a
              ...
              rel="noreferrer"
              data-cursor-hover
              className="group flex items-center gap-4 bg-cream p-5 text-left transition-all duration-300 hover:-translate-y-1"
              style={{
                border: "1px solid rgba(37,211,102,0.25)",
                boxShadow:
                  "0 10px 24px -16px rgba(37,211,102,0.2), inset 0 1px 0 hsl(0 0% 100% / 0.7)",
              }}
            >
              ...
              <div className="relative flex-shrink-0">
                <img
                  src={w.photo}
                  alt={w.name}
                  className="h-14 w-14 rounded-full object-cover ring-2 ring-[#25D366]/40 transition-all duration-300 group-hover:ring-[#25D366]"
                />
                <span
                  className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#25D366] text-white"
                >
              ...
              <div className="min-w-0 flex-1">
                <div className="font-mono-tag text-[10px] tracking-[0.2em] text-dark/50">
                  {w.role.toUpperCase()}
                </div>
                <div className="mt-0.5 font-display-bold text-lg text-dark transition-colors group-hover:text-[#128C7E]">
                  {w.name}
                </div>
                <div className="mt-0.5 font-mono text-xs text-dark/60">{w.value}</div>
              </div>
              {/* Arrow */}
              <svg className="h-4 w-4 flex-shrink-0 text-[#25D366] opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
